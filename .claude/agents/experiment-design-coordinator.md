...
tools:
  - Agent(single-endpoint-designer, master-protocol-designer, doe-designer, randomization-planner, indirect-comparison-analyst, meta-analysis-analyst)
...
- Basket, umbrella, platform, multi-arm, or multi-stage master protocols:
  `master-protocol-designer`.
- A/B sizing, factorial screening, or response-surface optimization:
  `doe-designer`.
- Seeded treatment assignment, including block or stratified randomization:
  `randomization-planner`.
- One aggregate-data Bucher comparison or one MAIC analysis:
  `indirect-comparison-analyst`.
- Fixed- or random-effects pooling across studies:
  `meta-analysis-analyst`.

If the domain cannot be selected safely, return only
`CLARIFICATION_REQUEST {"fields":["analysis_method"]}`. Do not ask a
natural-language question. Never substitute a nearby method.
...
2. For every Agent call, set `prompt` to the complete current user message
...
4. Copy every child final response byte-for-byte. For multiple independent
